...
Reference builds:
- `Asset/Output/ClearTax e-Invoicing Vendor Evaluation Framework/` - 3 copies, CTA on page 12 ("Get Your Country-Specific Exposure Assessment" pill).
- `Asset/Output/Decoding Oman's/` - 3 copies, CTA on page 12 ("Talk to an expert" pill).
...
Note: `SKILL.md` and `scripts/` in this folder have gone missing between sessions at least once before, while `.venv` survived. If either is missing when this skill is invoked, that's a sign this happened again - recreate them from this reference before proceeding, and mention it to the user.
...
3. If there's no existing link, locate the CTA button's rect. CTA buttons are almost always on the last page. Find the button text, then the filled pill shape it sits on:
   ...
   ```
   - Find the CTA phrase's approximate y-position first (e.g. `page.get_text()` or a quick per-page dump), so `--y-range` is a narrow window around it - a wildly wrong guess can also make the preview clip fall outside the page and error out; keep the range inside `[0, page.rect.height]`.
...
- Any number of UTM links is fine - one output file per link, regardless of whether the batch is 2, 3, or 5+ links.
