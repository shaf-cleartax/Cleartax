...
/* ===================================================================
   ...
   A sphere of nodes (countries / ERPs on the Peppol network) with
   arcing connections lighting up between them, echoing the
   planetary-scale visuals of breakthroughenergy.org's hero.
=================================================================== */
function initHero() {
  ...
  // arcing connections between random node pairs — "invoices" travelling the network
  const ARC_COUNT = 26;
  const arcLines = [];
  function makeArc() {
...
function initGsap() {
  ...
  // testimonial auto-scroll marquee-ish drift, plus reveal
...
function buildMarquees() {
  ...
  const badges = [
    ['GDPR', 'GDPR.svg'], ['ISO 27001', 'ISO.svg'], ['VAPT', 'VAPT.svg'], ['AICPA SOC', 'AICPA.svg'],
    ['ZATCA', 'ZATCA-logo.svg'], ['FTA', 'FTA.jpg'], ['Peppol', 'Peppol.svg'], ['IATA', 'IATA.svg'],
  ];
  const complianceTrack = document.getElementById('complianceTrack');
  if (complianceTrack) {
    // No lazy-loading here: this track's content is duplicated for a seamless
    // loop, and the CSS animation moves by a fixed -50% regardless of image
    // load timing — but lazy images popping in mid-scroll still looks broken.
    const html = badges.map(([name, file]) => `<span class="badge-chip"><img src="assets/img/compliance/${file}" alt="${name}"></span>`).join('');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // failsafe: never leave content permanently invisible, even if a tween silently stalls
